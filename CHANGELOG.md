...
### Added
- New upstream source: [AIScientists-Dev/academic-humanizer](https://github.com/AIScientists-Dev/academic-humanizer) (485 ⭐, MIT, 1 skill — `academic-humanizer`: improves the clarity and voice of AI-assisted academic prose and grant proposals (NSF Project Summary/Description, NIH Specific Aims) while preserving scholarly conventions, tying claims to evidence, and never altering a number, result, or citation).
- Placed in the academic/writing tier in `scripts/sync_skills.py`, after `nature-skills` and before `gdm-science` (`adaptyv-protein` retains its last-wins position). The skill ships as a single root-level `SKILL.md`, so its install directory takes the source's `local_name` (`academic-humanizer`); the name is unique, so it adds with **zero collisions**.
- Skill count after adding the source: 2,031 → **2,032** (+1 new). The `description: |` folded-scalar frontmatter block was normalized into a single quoted scalar by the sync sanitizer, parses cleanly under `yaml.safe_load` (458 chars, ≤ 1024), and passes the idempotency check.
- README: source table, highlights (Drafting row), acknowledgements, and badges updated (16 → 17 sources).
